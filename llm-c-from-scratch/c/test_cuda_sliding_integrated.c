...
int main(void) {
    ...
    if (!scratch || !x || !out || !wq_a || !wqa_n || !wq_b || !wkv || !wkvn || !sink || !wo_a || !wo_b) {
        return 1;
    }
    ...
    for (size_t i = 0; i < (size_t)T * (size_t)C; i++) {
        if (!isfinite(out[i])) {
            fprintf(stderr, "non-finite sliding output at %zu\n", i);
            ds4_config_free(&cfg);
            free(wo_b);
            free(wo_a);
            free(sink);
    ...
    if (ds4_cuda_available()) {
        printf("OK — sliding_attn forward with CUDA dispatch (devices=%d sum_abs=%.6g)\n", ds4_cuda_device_count(), sum);
    } else {
        printf("OK — sliding_attn forward with CPU cuda fallbacks (sum_abs=%.6g)\n", sum);
    }
