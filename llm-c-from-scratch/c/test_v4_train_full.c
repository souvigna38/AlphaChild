...
static void fill_layer(Ds4LayerWeights *w, const DeepSeekV4Config *cfg, int layer_idx, int hash) {
    ...
    if (cfg->layer_types[layer_idx] == DS4_ATTN_HCA) {
        ...
        w->hca_pos_bias = (float *)calloc((size_t)cfg->compress_rate_hca * (size_t)D, sizeof(float));
...
int main(void) {
    ...
    fill_layer(&mw.layers[0], &cfg, 0, 1);
    fill_layer(&mw.layers[1], &cfg, 1, 1);
    ...
    printf("train-full loss step0=%.4f step1=%.4f\n", loss0, loss1);
    if (!(loss1 <= loss0 + 0.15f)) {
    ...
    free(grad);
    ds4_config_free(&cfg);
    printf("OK — train-full (2-layer sliding+HCA, hash_moe backward)\n");
    return 0;
}
