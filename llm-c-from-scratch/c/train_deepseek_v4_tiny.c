...
static void usage(const char *prog) {
    fprintf(
        stderr,
        "Usage: %s [smoke] | -train-head STEPS | -train-adam STEPS | -train-1layer STEPS | -train-full STEPS [-lr LR] [-data path.txt]\n",
        prog);
}
...
int main(int argc, char **argv) {
    ...
    printf("=== DeepSeek-V4 C port (phases 0–11) ===\n");
    ...
    ds4_config_free(&cfg);
    printf("\nPhase 0–11 OK (forward + train-head + train-final + train-1layer + train-full)\n");
    return 0;
}
